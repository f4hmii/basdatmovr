...
// PERBAIKAN 1: Cek 'id' atau 'produk_id' agar lebih fleksibel
if (isset($_GET['id'])) {
    $produk_id = intval($_GET['id']);
} elseif (isset($_GET['produk_id'])) {
    $produk_id = intval($_GET['produk_id']);
} else {
    echo "<p>Produk tidak ditemukan (ID tidak ada di URL).</p>";
    exit;
}

if ($produk_id <= 0) {
    echo "<p>Produk tidak ditemukan (ID tidak valid).</p>";
    exit;
}

// Ambil detail produk
$stmt = $conn->prepare("SELECT p.*, s.nama_toko, k.nama_kategori FROM produk p LEFT JOIN seller s ON p.seller_id = s.pengguna_id JOIN kategori k ON p.kategori_id = k.kategori_id WHERE p.produk_id = ?");
...
if (!$product) {
    echo "<p>Produk tidak ditemukan di database untuk ID: $produk_id.</p>";
    exit;
}
...
$stmt_variants = $conn->prepare("SELECT pv.stock, w.nama_warna, u.nama_ukuran FROM produk_varian pv JOIN warna w ON pv.warna_id = w.warna_id JOIN ukuran u ON pv.ukuran_id = u.ukuran_id WHERE pv.produk_id = ? AND pv.stock > 0");
...
while ($variant = $result_variants->fetch_assoc()) {
    ...
    $stock = $variant['stock'];
    if (!in_array($color, $available_colors)) $available_colors[] = $color;
    if (!in_array($size, $available_sizes)) $available_sizes[] = $size;
...
<head>
    ...
    <style>
        .radio-btn-label { border: 2px solid #e2e8f0; padding: 0.5rem 1rem; border-radius: 0.375rem; cursor: pointer; transition: all 0.2s ease-in-out; }
        .radio-btn:checked + .radio-btn-label { border-color: #4f46e5; background-color: #e0e7ff; color: #4f46e5; }
        .thumbnail { cursor: pointer; border: 2px solid transparent; transition: border-color 0.2s; }
        .thumbnail:hover, .thumbnail.active { border-color: #4f46e5; }
    </style>
</head>
...
<div class="container mx-auto mt-10 p-4">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
            <div class="bg-white rounded-lg shadow-lg p-4"><img id="mainImage" class="w-full h-96 object-cover rounded-lg" src="../uploads/<?= htmlspecialchars($product['foto_url']) ?>" alt="Main Product Image"></div>
            <div class="flex space-x-2 mt-4">
                <img class="thumbnail active w-20 h-20 object-cover rounded-md" src="../uploads/<?= htmlspecialchars($product['foto_url']) ?>" onclick="changeImage(this)">
                <?php foreach ($detail_fotos as $foto): ?><img class="thumbnail w-20 h-20 object-cover rounded-md" src="../uploads/<?= htmlspecialchars($foto) ?>" onclick="changeImage(this)"><?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <form id="addToCartForm" class="mt-6">
                ...
                <input type="hidden" name="has_variants" value="<?= $has_variants ? '1' : '0' ?>">

                <?php if ($has_variants): ?>
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-gray-700 mb-2">Warna</h3>
                    <div class="flex flex-wrap gap-2"><?php foreach ($available_colors as $color): ?><input type="radio" name="color" id="color-<?= htmlspecialchars($color) ?>" value="<?= htmlspecialchars($color) ?>" class="hidden radio-btn"><label for="color-<?= htmlspecialchars($color) ?>" class="radio-btn-label"><?= htmlspecialchars($color) ?></label><?php endforeach; ?></div>
                </div>
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-gray-700 mb-2">Ukuran</h3>
                    <div class="flex flex-wrap gap-2"><?php foreach ($available_sizes as $size): ?><input type="radio" name="size" id="size-<?= htmlspecialchars($size) ?>" value="<?= htmlspecialchars($size) ?>" class="hidden radio-btn"><label for="size-<?= htmlspecialchars($size) ?>" class="radio-btn-label"><?= htmlspecialchars($size) ?></label><?php endforeach; ?></div>
                </div>
                <?php endif; ?>
                ...
                <div class="mb-6">
                    ...
                    <?php if ($has_variants): ?><p id="stockInfo" class="text-sm text-gray-500 mt-2">Pilih warna dan ukuran untuk melihat stok.</p><?php endif; ?>
                </div>

                <div class="flex space-x-4"><button type="submit" class="flex-1 bg-indigo-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-indigo-700 transition-colors">Tambah ke Keranjang</button></div>
            </form>
...
<script>
    ...
    // PERBAIKAN 2: Bungkus semua event listener dalam DOMContentLoaded
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function updateStockInfo() {
            ...
            if (stockInfo && selectedColor && selectedSize) {
                ...
                if (quantityInput.value > stock) quantityInput.value = stock > 0 ? stock : 1;
            }
        }
        ...
        if(incrementBtn) {
            incrementBtn.addEventListener('click', () => {
                ...
                let maxVal = parseInt(quantityInput.max) || 999;
                if (currentVal < maxVal) quantityInput.value = currentVal + 1;
            });
        }
